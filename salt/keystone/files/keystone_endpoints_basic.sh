...
MYSQL_DATABASE={{KEYSTONE_DBNAME}}
...
keystone service-create --name flow --type flow --description 'Flow Service'
...
create_endpoint () {
  case $1 in
    ...
    ;;
    flow)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$HOST_IP"':9771/v1/$(tenant_id)s' --adminurl 'http://'"$HOST_IP"':9771/v1/$(tenant_id)s' --internalurl 'http://'"$HOST_IP"':9771/v1/$(tenant_id)s'
    ;;
    #network)
...
for i in compute volume image object-store identity ec2 flow; do
