...
cinder-install:
  file.managed:
    ...
    - group: root
  cmd.run:
    - name: cd /usr/local/src && tar zxf cinder.tar.gz && cd cinder && pip install --no-index -f http://10.0.0.163/pip -r tools/pip-requires && python setup.py install
    - require:
...
python-cinderclient:
  file.managed:
    ...
    - group: root
  cmd.run:
    - name: cd /usr/local/src && tar zxf python-cinderclient.tar.gz && cd python-cinderclient && pip install --no-index -f http://10.0.0.163/pip -r tools/pip-requires && python setup.py install
    - require:
